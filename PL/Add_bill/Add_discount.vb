...
Public Class Add_discount
        ...
        Try
            If tbListData.Text <> String.Empty And txt_detail.Text <> String.Empty Then
                If MsgBox("ต้องการบันทึกข้อมูลใช่มั้ย ?", MsgBoxStyle.Question Or MsgBoxStyle.YesNo, "Confirm") = MsgBoxResult.Yes Then
                    ...
                    If BLL_Bill.InsertListBill(tbListData.Text, txt_detail.Text, "ข้อมูลส่วนลด") Then
                        MsgBox("บันทึกข้อมูลสำเร็จ", MsgBoxStyle.Information, "Information")
                        Clear()
                        Add_discount_Load(sender, e)
                    Else
    ...
    Private Sub Add_discount_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            btAdd.Enabled = True
            Dim BLL_Bill As BLL_BIll = New BLL_BIll
            Dim Dt As DataTable = BLL_Bill.SelectListBill("ข้อมูลส่วนลด")
            DGV.Rows.Clear()
